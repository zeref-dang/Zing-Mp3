...
const CardMusic = (props) => {
  ...
  return (
    <div className="cardMusic" onClick={() => onClick()}>
      <div className="inner">
        <div className={`left ${newRelease ? "left-100" : ""}`}>
          <div className="icon">{icon}</div>
          <div className={`img ${newRelease ? "img-60x60" : ""}`}>
            <img src={img} />
            <div className="dark">
          ...
          <div className="title">
            ...
            <div className="timeNewRelease">{timeNewRelease}</div>
          </div>
          {vip && <div className="vip">{VIP}</div>}
        </div>
        <div className={`center ${newRelease ? "center-0" : ""}`}>
          <div className="album">{album}</div>
        </div>
        <div className={`right ${newRelease ? "right-0" : ""}`}>
